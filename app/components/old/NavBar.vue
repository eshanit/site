...
<template>
  <header class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 font-sans" :class="[
    isScrolled
    ...
    <div class="w-full px-4 md:px-6 lg:px-8 xl:px-12">
      ...
      <div class="flex items-center justify-between">
            ...
            <li v-for="(item, index) in menuItems" :key="item.name" class="relative"
              ...
              <NuxtLink :to="item.href"
                ...
                <span
                  ...
                  :class="item.current ? 'w-full' : 'w-0 group-hover:w-full'" aria-hidden="true"></span>
              </NuxtLink>
              ...
              <Transition enter-active-class="transition-all duration-200 ease-out"
                ...
                <div v-if="item.submenu && openSubmenu === item.name"
                  class="absolute top-full left-0 mt-2 w-56 bg-white shadow-xl border border-gray-100 py-2 z-50"
                  role="menu" aria-label="Submenu">
                  <div
                    class="absolute -top-2 left-4 w-4 h-4 bg-white border-l border-t border-gray-100 transform rotate-45">
                  </div>
                  <NuxtLink v-for="subitem in item.submenu" :key="subitem.name" :to="subitem.href"
                    class="relative block px-5 py-3 text-sm transition-all duration-150 border-l-4" :class="[
                      subitem.current
                    ...
                    ]" role="menuitem" tabindex="0" @click="handleSubmenuClick(subitem.href)">
                    {{ subitem.name }}
                  </NuxtLink>
                </div>
              </Transition>
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="isSearchOpen = !isSearchOpen"
            ...
            <UIcon :name="isSearchOpen ? 'i-heroicons-x-mark' : 'i-heroicons-magnifying-glass'" class="w-5 h-5" />
          </button>
          ...
          <!-- Mobile menu button -->
          <button @click="toggleMenu" class="lg:hidden p-2.5 transition-all duration-200 hover:bg-gray-100"
            :class="isMenuOpen ? 'text-blue-700' : 'text-gray-600 hover:text-blue-700'"
            :aria-label="isMenuOpen ? 'Close main menu' : 'Open main menu'" :aria-expanded="isMenuOpen"
            :aria-controls="'mobile-menu'">
            <UIcon v-if="!isMenuOpen" name="i-heroicons-bars-3" class="w-6 h-6" aria-hidden="true" />
            <UIcon v-else name="i-heroicons-x-mark" class="w-6 h-6" aria-hidden="true" />
          </button>
        </div>
      </div>
      ...
      <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 max-h-0"
        enter-to-class="opacity-100 max-h-20" leave-active-class="transition-all duration-150 ease-in"
        leave-from-class="opacity-100 max-h-20" leave-to-class="opacity-0 max-h-0">
        <div v-if="isSearchOpen" class="lg:hidden overflow-hidden pb-4">
          <div class="relative">
            <input v-model="searchQuery" type="text" placeholder="Search..."
      ...
      <div v-if="isScrolled" class="h-0.5 bg-gradient-to-r from-brand-dark via-brand-light to-brand-dark origin-left"
        :style="{
          ...
          <div v-for="(item, index) in menuItems" :key="item.name" role="none">
            ...
            <div v-if="item.submenu">
              <button @click="toggleSubmenu(item.name)"
                ...
                ]" :aria-expanded="openSubmenu === item.name" role="menuitem" tabindex="0">
                <span class="font-semibold">{{ item.name }}</span>
                <UIcon name="i-heroicons-chevron-down" class="w-5 h-5 transition-transform duration-200"
                  :class="openSubmenu === item.name ? 'rotate-180' : ''" />
              </button>

              <!-- Mobile submenu items -->
              <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 max-h-0"
                enter-to-class="opacity-100 max-h-40" leave-active-class="transition-all duration-150 ease-in"
                leave-from-class="opacity-100 max-h-40" leave-to-class="opacity-0 max-h-0">
                <div v-if="openSubmenu === item.name" class="bg-gray-50 overflow-hidden" role="menu">
                  <NuxtLink v-for="subitem in item.submenu" :key="subitem.name" :to="subitem.href"
                    class="block pl-12 pr-6 py-3.5 text-sm transition-all duration-150 border-l-4" :class="[
                      subitem.current
                        ? 'text-blue-700 bg-blue-50 border-blue-700'
            ...
            <NuxtLink v-else :to="item.href"
              class="block px-6 py-4 text-base font-semibold transition-all duration-150 border-l-4" :class="[
                item.current
                  ? 'text-blue-700 bg-blue-50 border-blue-700'
                  : 'text-gray-700 hover:text-blue-700 hover:bg-gray-50 border-transparent'
              ]" @click="closeMobileMenu" @keydown="(e) => handleMenuKeydown(e, index)" role="menuitem" tabindex="0">
              {{ item.name }}
            </NuxtLink>
          </div>
        </nav>
